...
def test_agg_base_stats_simple():
    ...
    assert line.base_stats.attack == pytest.approx(100)
    assert line.base_stats.health == pytest.approx(200)
    assert line.base_stats.lethality == pytest.approx(10)
    assert line.base_stats.defense == pytest.approx(10)
...
def test_agg_base_stats_multiple_tiers():
    ...
    assert 206 < line.base_stats.attack < 243
    assert 619 < line.base_stats.health < 730

    # make sure the combined attack and health are as expected.
    # base lehtality and def is always 10.
    assert line.base_stats.attack == pytest.approx(226.93800)
    assert line.base_stats.health == pytest.approx(681.81063)
    assert line.base_stats.lethality == pytest.approx(10)
    assert line.base_stats.defense == pytest.approx(10)
